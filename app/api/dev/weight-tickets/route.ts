...
import { consumeRateLimit, acquireConcurrencySlot } from '@/lib/server/rateLimit';
...
export async function POST(request: NextRequest) {
    ...
    try {
        const rateLimit = await consumeRateLimit('dev-ocr', 'dev-temp', 100, 15 * 60 * 1000);
        ...
        try {
            releaseSlot = await acquireConcurrencySlot('dev-ocr', MAX_CONCURRENT_OCR_REQUESTS, N8N_TIMEOUT_MS + 5_000);
            ...
            if (!upstreamResponse.ok) {
                console.error('[dev-weight-tickets] n8n upstream failed', upstreamResponse.status);
            ...
            if (releaseSlot) {
                try {
                    ...
                    console.error('[dev-weight-tickets] concurrency lease release failed');
                }
            }
        }
    } catch (error) {
        ...
        if (message === 'N8N_WEBHOOK_URL_INVALID') {
            console.error('[dev-weight-tickets] n8n webhook configuration invalid');
            return NextResponse.json({ error: 'Dịch vụ OCR chưa được cấu hình an toàn.' }, { status: 503 });
        }

        console.error('[dev-weight-tickets] request failed');
